...
class JSRT_Loader(Sequence):
    def __init__(self, folder_path, folds=['fold1'], image_size=(320,240), batch_size=4, mode='seg', target_classes=["Good Crypts"], filter_classes=[], augment=True):
        """
        ...
        self.image_ids = []
        for fold in folds:
            self.image_ids.extend(os.listdir(os.path.join(folder_path, 'annotations', fold, 'masks', 'heart')))
        self.image_ids = [imgid.replace('.gif', '') for imgid in self.image_ids]
    ...
    def load_image(self, index):
        ...
        Returns PIL image
        """
        image_path = os.path.join(self.folder_path, 'images', self.image_ids[index]+'.png')
        img = Image.open(image_path)
        img = ImageOps.grayscale(img)
        img_arr = np.expand_dims(np.array(img), axis=-1)
        img = np.concatenate([img_arr]*3, axis=-1)
    ...
    def load_annotations(self, index):
        ...
        """
        # mask_file = self.image_ids[index].replace('dcm', 'png').replace('DICOM_anon', 'Ground')
        # mask_img = Image.open(mask_file)
        masks = {}
        for label in self.target_classes:
            label_mask_path = None
            for fold in self.folds:
                curr_path = os.path.join(self.folder_path, 'annotations', fold, 'masks', label, self.image_ids[index]+'.gif')
                if os.path.isfile(curr_path):
                    label_mask_path = curr_path
                    break
    ...
    def get_instance(self, index):
        ...
        masks_gt = self.load_annotations(index)
        w, h = list(masks_gt.items())[0][1].size
        img = img.resize((w, h))
        ...
        masks['heart'] = masks_gt['heart']
        clavicle_union = np.clip(np.array(masks_gt['left clavicle'])/255. + np.array(masks_gt['right clavicle'])/255, 0, 1)
        masks['clavicle'] = Image.fromarray(clavicle_union.astype(np.uint8) * 255)
        lung_union = np.clip(np.array(masks_gt['left lung'])/255. + np.array(masks_gt['right lung'])/255, 0, 1)
        masks['lung'] = Image.fromarray(lung_union.astype(np.uint8) * 255)
        ...
        if debug:
            ...
            debug_image[:, :, 0] = np.array(masks['heart'])
            debug_image[:, :, 1] = np.array(masks['clavicle'])
            debug_image[:, :, 2] = np.array(masks['lung'])
            debug_image = Image.fromarray(debug_image)
            debug_image.show()

        # Perform random augmentations
        if self.augment:
            img, masks, bboxes = self.augment_instance(img, masks, bboxes, flip_ver=0, rotate_90=0)
...
if __name__ == "__main__":
    ...
    # train_path = "D:/Master/Study/Semester8/BTP2/CHAOS_train/MR"
    # val_path = "D:/Master/Study/Semester8/BTP2/CHAOS_train/MR"
    folder_path = "D:/Master/Study/Semester8/BTP2/JSRT_dataset/jsrt"
    ...
    train_gen = JSRT_Loader(folder_path, ['fold1'], img_size, batch_size, mode='seg', target_classes=target_classes
