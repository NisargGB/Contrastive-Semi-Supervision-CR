...
class CHAOS_Loader(Sequence):
    def __init__(self, image_ids, image_size=(320,240), batch_size=4, mode='seg', target_classes=["Good Crypts"], filter_classes=[], augment=True):
        """
        ...
        #     dir_pth = os.path.join(folder_path, patient, 'T2SPIR', 'DICOM_anon')
        #     imgs = os.listdir(dir_pth)
        #     imgs = [os.path.join(dir_pth, img) for img in imgs]
        #     self.image_ids.extend(imgs)
    ...
    def load_image(self, index):
        ...
        mri_dicom = dcmread(image_path)
        img = mri_dicom.pixel_array
    ...
    def augment_instance(self, img, masks, bboxes, flip_hor=None, flip_ver=None, rotate_90=None, brightness_factor=None, contrast_factor=None):
        ...
        window_factor = 1. - random.random() * 0.3
    ...
    def get_instance(self, index):
        ...
        masks = {label: None for label in self.target_classes}
        
        # Drawing the masks
        anns_arr = np.array(anns)
        for label in self.target_classes:
            class_mask = (anns_arr >= self.pixel_values[label][0]) * (anns_arr <= self.pixel_values[label][1]) * 1
            class_mask = (class_mask * 255).astype(np.uint8)
            class_mask = Image.fromarray(class_mask)
            masks[label] = class_mask
        ...
        if debug:
            ...
            debug_image[:, :, 0] = np.array(masks['Liver'])
            debug_image[:, :, 1] = np.array(masks['Left kidney'])
            debug_image[:, :, 2] = np.array(masks['Right kidney'])
            if "Spleen" in masks:
                debug_image += np.expand_dims((np.array(masks["Spleen"])//2), -1)
            debug_image = Image.fromarray(debug_image)
            debug_image.show()

        # Perform random augmentations
        if self.augment:
            img, masks, bboxes = self.augment_instance(img, masks, bboxes, flip_hor=0, flip_ver=0, rotate_90=0)
...
if __name__ == "__main__":
    ...
    train_image_ids = image_ids[:(4 * num_imgs // 5)]
    val_image_ids = image_ids[(num_imgs // 5):]
    ...
    train_gen = CHAOS_Loader(train_image_ids, img_size, batch_size, mode='seg', target_classes=target_classes
